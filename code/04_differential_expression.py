...
for cl in clusters:
    ...
    print(
        f"  {cl}: "
        f"{len(up)} up-regulated, "
        f"{len(dn)} down-regulated "
        f"(FDR<5%, |FC|>0.5)"
    )

    all_degs[cl] = result

    DEG_SAVE_PATH = os.path.join(
        OUTDIR,
        f"DEG_{cl}.csv"
    )
    ...
    print(f"[Saved] {DEG_SAVE_PATH}")

# ── PAM50 overlap ──────────────────────────────────────────────────────────
print("\nPAM50 gene availability in expression matrix:")

pam50_avail = [
    g for g in PAM50_GENES
    if g in expr.columns
]

print(
    f"  {len(pam50_avail)}/{len(PAM50_GENES)} "
    f"PAM50 genes found"
)
...
for cl in clusters:
    ...
    pam50_deg = deg.loc[
        [
            g for g in pam50_avail
            if g in deg.index
        ]
    ]

    top10 = pam50_deg.nsmallest(
        10,
        'p_value'
    )[
...
hmap_sorted = hmap_z[sorted_cols]
...
ax.set_title(
    "Gene Expression Heatmap — Top DEGs per Cluster\n"
...
fig.suptitle(
    "Volcano Plots — One-vs-Rest Differential Expression",
    fontsize=13,
    fontweight='bold'
)
...
for ax, cl in zip(axes, clusters):
    ...
    ax.scatter(
        deg.loc[~sig_mask, 'log2FC'],
        deg.loc[~sig_mask, '-log10p'],
        s=2,
        alpha=0.3,
        color='gray',
        label='NS'
    ...
    ax.scatter(
        deg.loc[up_mask, 'log2FC'],
        deg.loc[up_mask, '-log10p'],
        s=6,
        alpha=0.6,
        color='#d62728',
    ...
    ax.scatter(
        deg.loc[dn_mask, 'log2FC'],
        deg.loc[dn_mask, '-log10p'],
        s=6,
        alpha=0.6,
        color='#1f77b4',
