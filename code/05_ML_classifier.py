...
le = LabelEncoder()
y = le.fit_transform(clin['cluster_label'].values)
print(f"Classes: {le.classes_}, n={len(y)}")

# ── Use top-500 DEGs as features ─────────────────────────────────────────────
top_feat_genes = []
for cl in clusters:
    deg = pd.read_csv(f"{OUTDIR}/DEG_{cl}.csv")
    sig = deg[(deg['p_adj'] < 0.05) & (deg['log2FC'].abs() > 0.5)]
    top_feat_genes.extend(sig.nsmallest(200, 'p_value')['gene'].tolist())
top_feat_genes = list(dict.fromkeys(top_feat_genes))[:500]
top_feat_genes = [g for g in top_feat_genes if g in expr.columns]
...
y_pred_proba = cross_val_predict(rf, X_scaled, y, cv=cv, method='predict_proba')
...
y_bin = label_binarize(y, classes=range(len(clusters)))
aucs = {}
...
y_lr_proba = cross_val_predict(lr, X_pca[:, :20], y, cv=cv, method='predict_proba')
y_lr_class = y_lr_proba.argmax(axis=1)
lr_acc = accuracy_score(y, y_lr_class)
lr_aucs = {}
for i, cl in enumerate(clusters):
    a = roc_auc_score(y_bin[:, i], y_lr_proba[:, i])
    lr_aucs[cl] = a
print(f"  LR Accuracy: {lr_acc:.4f}, Mean AUC: {np.mean(list(lr_aucs.values())):.4f}")

# ── Feature importance ────────────────────────────────────────────────────────
print("\nFitting final RF on full data for feature importance...")
rf.fit(X_scaled, y)
importances = pd.Series(rf.feature_importances_, index=top_feat_genes)
top50_imp = importances.nlargest(50)
print("Top 10 prognostic genes by RF importance:")
for g, v in top50_imp.head(10).items():
    print(f"  {g}: {v:.5f}")

top50_imp.to_csv(f"{OUTDIR}/rf_feature_importance.csv", header=['importance'])

# ── Concordance index (C-index) for survival risk score ──────────────────────
# Risk score = probability of high-risk cluster (C3 had highest event rate)
high_risk_cl = clusters.index('C3') if 'C3' in clusters else 0
...
def concordance_index(time, event, risk):
    ...
    return concordant / denom if denom > 0 else 0.5

# Sample 500 patients for speed
np.random.seed(42)
sample_idx = np.random.choice(len(risk_score), size=min(500, len(risk_score)), replace=False)
cindex = concordance_index(
    clin['OS.time'].values[sample_idx],
    clin['OS'].values[sample_idx],
    risk_score[sample_idx]
)
print(f"\nHarrell's C-index (OS, n=500 sample): {cindex:.4f}")
print("  (0.5=random, >0.7=good, >0.8=excellent)")

# ── FIGURE: ROC curves ───────────────────────────────────────────────────────
colors = ['#1f77b4', '#d62728', '#2ca02c', '#9467bd']
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle("TCGA-BRCA: ML Classifier Performance", fontsize=13, fontweight='bold')
...
for i, cl in enumerate(clusters):
    ...
    ax.plot(fpr, tpr, lw=2, color=colors[i], label=f'{cl} AUC={aucs[cl]:.3f}')
ax.set_xlabel("False Positive Rate", fontsize=11)
ax.set_ylabel("True Positive Rate", fontsize=11)
ax.set_title(f"Random Forest ROC\n(5-fold CV, mean AUC={mean_auc:.3f})", fontsize=11)
...
ax.barh(range(15), top15.values[::-1], color=colors_bar, edgecolor='white')
ax.set_yticks(range(15))
ax.set_yticklabels(top15.index[::-1], fontsize=10)
ax.set_xlabel("Feature Importance (Gini)", fontsize=11)
ax.set_title("Top 15 Prognostic Genes\n(Random Forest Feature Importance)", fontsize=11)
ax.grid(axis='x', alpha=0.3)

plt.tight_layout()
plt.savefig(f"{FIGDIR}/05_ML_performance.png", dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(f"{FIGDIR}/05b_confusion_matrix.png", dpi=150, bbox_inches='tight')
plt.close()

# ── Save performance metrics ─────────────────────────────────────────────────
metrics = {
    'model': ['RandomForest', 'LogisticRegression'],
    'accuracy': [round(acc,4), round(lr_acc,4)],
    'mean_AUC': [round(mean_auc,4), round(np.mean(list(lr_aucs.values())),4)],
    'C_index_OS': [round(cindex,4), 'NA'],
}
pd.DataFrame(metrics).to_csv(f"{OUTDIR}/ml_performance.csv", index=False)

print(f"\n[Saved] {FIGDIR}/05_ML_performance.png")
print(f"[Saved] {FIGDIR}/05b_confusion_matrix.png")
